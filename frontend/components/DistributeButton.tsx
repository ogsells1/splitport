    ...
    return (
      <div className="space-y-1.5">
        <button
          ...
          disabled={!hasBalance}
          className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-white border border-stone-200 hover:bg-stone-50 disabled:opacity-50 disabled:cursor-not-allowed text-stone-700 font-medium rounded-xl transition-colors text-sm"
        >
        ...
        {!hasBalance && (
          <p className="text-xs text-stone-400 text-center">No pending balance</p>
        )}
        {hasBalance && (
          <p className="text-xs text-emerald-600 text-center">{pendingFormatted} USDC pending</p>
        )}
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-stone-900">Distribute USDC</h3>
          <button
        ...
        <div className="flex gap-1 p-1 rounded-lg bg-stone-100">
          <button
            ...
            className={`flex-1 py-1.5 rounded-md text-sm font-medium transition-colors ${
              mode === "full" ? "bg-white text-stone-900 shadow-sm" : "text-stone-500"
            }`}
          >
            Full amount
          </button>
          <button
            ...
            className={`flex-1 py-1.5 rounded-md text-sm font-medium transition-colors ${
              mode === "custom" ? "bg-white text-stone-900 shadow-sm" : "text-stone-500"
            }`}
          >
            Custom amount
          </button>
        ...
        {mode === "custom" && (
          <div>
            ...
            <button
              ...
              className="mt-1.5 text-xs text-emerald-600 hover:text-emerald-700"
            >
            ...
            <p className="text-xs text-stone-400 mt-2 bg-stone-50 rounded-lg px-3 py-2">
