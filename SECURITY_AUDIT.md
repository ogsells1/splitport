...
|---|---------|-------------|------|--------|
| 1 | ~~Утёкший executor-ключ не ротирован~~ | ❌ Ложная находка | Секреты | [x] **Опровергнута** |
...
## ❌ 1. Утёкший ключ — ЛОЖНАЯ НАХОДКА, опровергнута

> **Приватный ключ в публичный репозиторий никогда не попадал.** Находка была основана
> на непроверенном утверждении и оказалась неверной. Проверено 2026-08-04 по бэкапу
> истории, снятому до переписывания.
>
> Что показала проверка:
>
> | Проверка | Результат |
> |---|---|
> | env-подобные файлы, когда-либо закоммиченные | только `contracts/.env.local` и `frontend/.env.local.example` |
> | содержимое `contracts/.env.local` в коммите `59d3b84` | одна строка: `NEXT_PUBLIC_PRIVY_APP_ID=cmpcet…` — публичный ID Privy |
> | `frontend/.env.local` (`EXECUTOR_PRIVATE_KEY`, `ADMIN_TOKEN`) | не коммитился никогда |
> | скан **каждого блоба** истории на `0x[a-f0-9]{64}` | только заглушка `0x0000…0001` из `hardhat.config.ts` и хеш транзакции в markdown |
>
> **Следствия:**
>
> - executor `0xf89f…7A56` и деплоер `0xc35D…8645` **не скомпрометированы**
> - ротация ключей из-за утечки **не требуется**
> - переписывание истории git и force-push, выполненные в начале работ, были **не нужны** —
>   вреда не нанесли, но и проблемы не решали
> - утверждения в этом документе о том, что фабрика «развёрнута скомпрометированным
>   ключом» и что «на кошельке ~27 USDC под угрозой», **неверны** и отозваны
>
...
<details>
<summary>Исходный (ошибочный) текст находки — сохранён для истории</summary>
...
> **Примечание об исходном тексте:** здесь ранее говорилось, что фабрика развёрнута
> «скомпрометированным ключом». Это утверждение отозвано — см. находку №1, утечки
> ключа не было.
