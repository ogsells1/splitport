...
- `projects` — id, name, contractAddress (уникальный), usdcAddress, chainId, deployBlock, ownerId → users, **splitMode** (PERCENTAGE по умолчанию | FIXED; задаётся при создании, неизменяем)
- `contributors` — id, projectId, wallet (nullable — pending-инвайт без кошелька), percentage (basis points, для PERCENTAGE), **fixedAmount** (USDC 6 dec, для FIXED), role, active, totalPaid, status (PENDING/CLAIMED), inviteToken (uniq), claimedByPrivyId (никогда не отдаётся в owner-facing API)
...
## Режим выплат проекта: PERCENTAGE vs FIXED ✅
При создании проекта (`/create`) выбирается `splitMode` (неизменяем):
- **PERCENTAGE** (как было) — у контрибьюторов `percentage` (bps, сумма = 100%); Distribute делит введённую сумму по %.
- **FIXED** — у каждого контрибьютора своя `fixedAmount` (USDC). Distribute платит каждому его сумму; total = Σ фикс-сумм.
- **Общий расчёт долей** в `lib/distribute.ts` → `computeShares(splitMode, contributors, {amountUsdc?, contributorIds?})`: PERCENTAGE требует amount и bps=100%; FIXED берёт `fixedAmount` (опц. подмножество по `contributorIds`). Используется в `runDistribution` (manual/cron) и `streams` POST.
- **Manual Distribute (FIXED)**: на дашборде — чекбоксы у участников + кнопки **«Distribute to all»** (Σ всех) и **«Distribute selected»** (только отмеченные). `POST /api/treasury/distribute` принимает `contributorIds`, amount опционален.
- **Авто/one-off/streaming в FIXED**: сумма выводится из фикс-сумм (поле ввода скрыто). `schedule`/`payments` POST в FIXED берут Σ фикс-сумм; `streams` POST — Σ через `computeShares`; cron `runDistribution` для FIXED игнорирует stored amount.
- **Редактирование фикс-сумм** (режим неизменяем, суммы — да): `PATCH /api/contributor` (owner-only, только FIXED) → инлайн-правка на дашборде.
- API, протянувшие режим: `project/create` (splitMode + amount per row), `project` GET (splitMode, fixedAmount), `invite` POST (amount в FIXED), `invite/[token]` GET (показ суммы). UI: `/create` (тогглер режима), `DbProjectDashboard` (передаёт splitMode+fixedTotal в Auto/Scheduled/Stream rows), invite-страница.
